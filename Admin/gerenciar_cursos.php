...
<head>
    ...
    <style>
        ...
        /* Estilo para limitar a descrição e adicionar reticências */
        .description-cell {
    ...
    <main class="container">
        <div class="content-card">
             <div class="card-header-flex">
                 <h2><i class="fa-solid fa-graduation-cap"></i> Gerenciamento de Cursos</h2>
                 <a href="cadastrar_curso.php" class="btn-add">
                     <i class="fa-solid fa-plus"></i> Adicionar Novo Curso
                 </a>
            </div>

            <?php if (isset($_GET['erro']) && $_GET['erro'] == 'dependencia'): ?>
                <div class="alert-danger">Erro: Não foi possível excluir o curso. Ele possui disciplinas ou turmas vinculadas.</div>
            <?php endif; ?>
                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                            <tr>
                                ...
                                <td class="description-cell" title="<?php echo htmlspecialchars($row['descricao']); ?>">
                                    <?php echo htmlspecialchars($row['descricao']); ?>
                                </td>
                                <td class="action-links">
                                    <a href="editar_curso.php?id=<?php echo $row['id_curso']; ?>" class="edit-link"><i class="fa-solid fa-pen-to-square"></i> Editar</a> 
                                    <a href="excluir_curso.php?id=<?php echo $row['id_curso']; ?>" onclick="return confirm('ATENÇÃO: Excluir este curso irá afetar todas as disciplinas e turmas vinculadas. Tem certeza?')" class="delete-link"><i class="fa-solid fa-trash-can"></i> Excluir</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
            ...
            <?php else: ?>
                <p class="no-data-message">Nenhum curso cadastrado no momento.</p>
            <?php endif; ?>
        </div>
    </main>
</body>
